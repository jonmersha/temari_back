...
//-----All Data Return from list of tables
router.get(
  "/data/:tableId",
...
router.get(
  "/posted/:tableId/:interval",
  apiKeyMiddleware.apiKeyMiddleware,
  async (req, res) => {
    const intervals = req.params.interval;
    const tableId = req.params.tableId;
    const stm = `SELECT * FROM ${table[tableId]} WHERE ${created_at} >= CURDATE() - INTERVAL ${intervals} DAY;`; //Query.selectAllCount(data[id]);
